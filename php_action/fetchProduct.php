...
$sql = "SELECT product.product_id, product.product_name, product.product_image, product.brand_id,
		...
		WHERE product.status = 1 AND product.quantity>0 AND product.companyid='$companyid'   ";
...
if ($result->num_rows > 0) {
	...
	while ($row = $result->fetch_array()) {
		...
		if ($row[7] == 1) {
			// activate member
			if ($row['quantity'] < 20) {
				$active = "<label class='label label-success' style='background-color:red'>Warning Quantity less than 20 </label>";
			} else {
