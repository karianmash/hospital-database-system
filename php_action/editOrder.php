	...
	if ($readyToUpdateOrderItem) {
		...
		for ($x = 0; $x < count($_POST['productName']); $x++) {
			...
			while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
				...
				// add into order_item
				$orderItemSql = "INSERT INTO order_item (order_id, product_id, quantity, rate, total, order_item_status,companyid)
				VALUES ({$orderId}, '" . $_POST['productName'][$x] . "', '" . $_POST['quantity'][$x] . "', '" . $_POST['rateValue'][$x] . "', '" . $_POST['totalValue'][$x] . "', 1,'$companyid')";
