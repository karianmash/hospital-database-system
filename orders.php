...
<div class="" style="width: 100%;text-align:center;background-image: linear-gradient( 95.2deg,  rgba(173,252,234,1) 26.8%, rgba(192,229,246,1) 64% );">
	<p style="color:black;font-size: 50px">Orders</p>
</div>
		...
		?>
			...
			<form class="form-horizontal" method="POST" action="php_action/createOrder.php" id="createOrderForm">
				...
				<div>
						...
						<tbody>
							...
							for ($x = 1; $x < 4; $x++) { ?>
								<tr id="row<?php echo $x; ?>" class="<?php echo $arrayNumber; ?>">
									...
									<td style="padding-left:20px;">
										<div class="form-group">
											<p id="available_quantity<?php echo $x; ?>" class="available_quantity"></p>
										</div>
									</td>
									<td style="padding-left:20px;">
										<div class="form-group">

											<input type="number" name="quantity[]" class="quantity" id="quantity<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control" min="1" />
										</div>


										<input type="text" class="conf" placeholder="confirm amount" class="form-control">
										<p class="er" style="color: red">Amount of product not enough</p>
										<script type="text/javascript">
											var er = document.querySelectorAll('.er');
											for (x = 0; x < er.length; ++x) {
												er[x].style.display = "none";
											}

											function val() {
												var q = document.querySelectorAll('.available_quantity');

												var va = document.querySelectorAll('.conf');
												for (i = 0; i < va.length; ++i) {
													for (j = 0; j < q.length; ++j) {
														if (va[i].value < q[i].innerHTML) {
															for (x = 0; x < er.length; ++x) {
																er[x].style.display = "none";
															}
															...
															for (x = 0; x < er.length; ++x) {
																er[x].style.display = "";
																location.href = 'http://localhost/adb/orders.php?o=add';
															}
					...
					$ses = $_SESSION['userId'];
					// print_r($ses);
					$sqltest = "SELECT * FROM orders WHERE user_id='$ses' and payment_status != '1'";
					...
					if ($querytest->num_rows > 0) {
				 	...
				 	document.querySelector(".panel-body").style.display = "none";
						...
						echo "<script>alert('You have pending or unpaid items: Sorry you cannot make any more orders untill you are cleared!')</script>";
					} else {
						echo '<script>
					...
					<div class="form-group submitButtonFooter">
						<div class="col-sm-offset-2 col-sm-10">
							...
							<button type="submit" id="createOrderBtn" data-loading-text="Loading..." class="btn btn-success" onclick="val()"><i class="glyphicon glyphicon-ok-sign"></i> Save Changes</button>
